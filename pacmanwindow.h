...
class PacmanWindow : public QFrame
{
  ...
  public:
    ...
    void handleTimer();
    void handleHandleTimer();   // connecte handleTimer à QTimer
    ...
    bool isGagner();                        // verifier si le joueur est gagné (manger tous les gommes)
    ...
    void gagner(int, int);                  // afficher le point de chaque joueur
                                            // arrêter le jeu et demander si le joueur a envie de rejoueur ou de sortir
    void perdu(int, int);                   // afficher le point de chaque joueur
                                             // arrêter le jeu et demander si le joueur a envie de rejoueur ou de sortir

    void sortir();                          // fermer la fenêtre PacmanWindow
    void rejouer();                         // initialize le jeu et ouvrir IntroBox

    void introduction();                    // IntroBox demander si le joueur veut jouer en mode seul ou à 2 joueurs
