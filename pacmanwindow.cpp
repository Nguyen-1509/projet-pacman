...
bool is_handle_timer_is_running = false;    // Vérifier si le handleTimer est entrain d'être connecté avec QTimer
                                            // Eviter de dupliquer la connection entre handleTimer et QTimer
                                            // (ca va augmenter la vitesse du jeu) fh
...
void PacmanWindow::paintEvent(QPaintEvent *)
{
	...
	if (isVivant(jeu.pacman1))
        painter.drawPixmap(jeu.pacman1.getPacmanX()*largeurCase, jeu.pacman1.getPacmanY()*hauteurCase+80, pixmapPacman1);
    // Dessine Pacman 2
    if (jeu.getNomJoueur() == 2)
        if (isVivant(jeu.pacman2))
            painter.drawPixmap(jeu.pacman2.getPacmanX()*largeurCase, jeu.pacman2.getPacmanY()*hauteurCase+80, pixmapPacman2);

	// Dessine Coeur Pacman 1
	for(int i=1;i<=jeu.pacman1.getPointDeVie();i++)
        painter.drawPixmap(400 + 40*(i-1), 5, pixmapCoeur);
    // Dessine Coeur Pacman 2
    if (jeu.getNomJoueur() == 2)
        for(int i=1;i<=jeu.pacman2.getPointDeVie();i++)
            painter.drawPixmap(400 + 40*(i-1), 45, pixmapCoeur);

    // Dessine Point Pacman 1
    painter.drawPixmap(250, 5, pixmapPacman1Fixe);
    // Dessine Point Pacman 2
    if (jeu.getNomJoueur() == 2)
        painter.drawPixmap(250, 45, pixmapPacman2Fixe);
...
void PacmanWindow::keyPressEvent(QKeyEvent *event)
{
    ...
    if (isVivant(jeu.pacman1)){                 // Verifier si le joueur est vivant
                                                // Si il est mort, il ne peut déplacer le pacman
...
bool PacmanWindow::isCollision(Jeu::Pacman &pacman)
{
    ...
    for (itFantome=jeu.fantomes.begin(); itFantome!=jeu.fantomes.end(); itFantome++)
    {
        if (pacman_x == itFantome->getPosX() && pacman_y == itFantome->getPosY())
            return true;
    }
    return false;
}
...
void PacmanWindow::collisionHandle()
{
    ...
    if (jeu.getNomJoueur()==1 && isVivant(jeu.pacman1)==false)
...
void PacmanWindow::mangerHandle()
{
    ...
    } else if (jeu.getNomJoueur() == 2)
    {
        if (isMangerBonus(jeu.pacman2))
        {
            ...
            if (jeu.pacman2.getPointDeVie() < 5)
                jeu.pacman2.setPointDeVie(jeu.pacman2.getPointDeVie() + 1);
        }
    }
}
...
{
    if (isGagner())
        gagner(jeu.pacman1.getPoint(), jeu.pacman2.getPoint());
}
